...
Java**没**有**无符号**的整数类型，包括 byte, shot, int, long
...
+ float, 32 bits / 4 Bytes, ![](./images2/Java-Float-range.svg) 6~7位有效数字
+ double, 64 bits / 8 Bytes, ![](./images2/Java-Double-range.svg) 15位有效数字, 默认小数类型
...
+ 主要类和接口
  + Class 类
    ...
    + Method:
      ...
      + `getDeclaredConstructors()` 获取所有声明的构造函数，包括私有构造
  ...
  + Constructor 类
    ...
    + Methods:
      + `newInstance(Object... initargs)` 使用指定的构造函数参数创建一个
...
+ 全方位的企业级应用开发框架(JaveEE / J2EE)，开源 + 轻量级
...
+ 类型
  ...
  + application.properties
    ...
    + ref: [Spring Boot / Common Application Properties](https://docs.spring.io/spring-boot/appendix/application-properties/index.html)
...
+  讨论
  ...
  + 作用
    + 隔离环境
      如，可分为 application-prod.properties, application-dev.properties, application-test.  properties
    + 代码和数据分离

###### Spring Boot 启动

+ Spring Boot 启动流程
  提供 "starter"起步依赖，简化构建配置。 可根据需要选择。
...
+ 程序入口
  + `main()`
    ...
    + 说明
      1. `new()`
         + 判断应用类型
           + 判断类路径中有无"javax.servlet"
             + 有，web应用
             + 无
             
      2. `StringApplication.run()`
         真正的启动
...
+ 启动五步骤
  ...
  1. 准备环境
     + 加载配置文件、系统变量、命令行参数
     + 封装为"ConfigurableEnvionment"
  2. 创建上下文环境，即 "ApplicationContext"对象 或 "上下文容器"
     装配Bean，加载配置类
     方式有 `@ComponentScan`, `@Import`, `@ImportSource`, etc
  3. 刷新/加载上下文
     + 所有 Bean 初始化完成
     + 事件监听器 注册完成
     + Web 项目 ？启动内嵌Tomcat : NoAction;  
  4. 调用接口 "CommandLineRunner" 和 "ApplicationRunner"
     + 启动后须完成的逻辑
  5. 发布 "ApplicationReadyEvent"，即 启动业务

+ "starter"
  + 说明
    + "Starter"是预定义的依赖描述符。
      本质上是 Maven/Gradle的一个模块，在依赖文件(pom.xml/build.gradle)中，预先声明了指定功能的所需依赖
    + 默认配置
    ...
    + ${ProjectName}
      ...
      + [D] src
        + [D] main
          + [D] java
            + [D] ...
              + [f] ${ProjectName}Application.java 
                项目启动类
          + [D] resources
            + [D] static
            + [D] templates
...
+ Spring
  + [spring boot](https://docs.spring.io/spring-boot/index.html)
    + [Appendix]()
      + [公用应用属性](https://docs.spring.io/spring-boot/appendix/application-properties/index.html)
...
+ 《Java核心编程 卷I 12Ed / 机械工业出版社 / ISBN:978-7-111-70641-0》

#### Spring Boot 3核心技术与最佳实践

周红亮 / 电子工业出版社 / ISBN:978-7-121-45290-1
...
##### [77sindu](https://space.bilibili.com/3546955412146263?spm_id_from=333.788.upinfo.etail.click)
...
+ [Java EE企业级应用开发教程（Spring+Spring MVC+MyBatis）（第3版）](https://www.ilibili.com/video/BV1BzwSzoEfq/?spm_id_from=333.788.recommend_more_video.0&rackid=web_related_0.router-related-2479604-6dnm7.1776137399665.556&d_source=38fc599412349dcfe60484e3ff320c66)
