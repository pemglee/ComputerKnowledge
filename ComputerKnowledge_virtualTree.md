# Computer Knowledge 笔记结构

## Virtual Tree

+ [(root)README](README.md)  

  + [OS 操作系统]  

    + [Windows]  

    + [~~Unix~~]  

    + [Linux](./Linux-ops.md)  

      + [Linux目录、文件学习笔记](./Linux-Directories_Files.md)  

      + [Linux命令学习笔记](./Linux-Commands.md)  

  + [网络知识]  

    + [TCP/IP](./Network-TCPIP.md)

  + [计算机编程]  

    + [C]

      + [C 知识点](./C_Point-list.md)  

    + [C++](./CPP-Overview.md)  

      + [C++ 编译和文件组织]

        + [GCC](./CPP-compiling.md)

          + [windows下GCC]

            + [MinGW](./CPP_MINGW-setup.md)

        + [MS VC]

        + [make]

        + [CMake](./CPP_CMake-2512.md)

      + [C++ 关键字](./CPP-keywords_list.md)  

      + [C++ 内建数据类型](./CPP-DataType.md)  

      + [C++ 流程控制](./CPP-FlowControl.md)  

      + [C++ 程序块(函数，过程，方法)]  

      + [C++ 指针和引用](./CPP-Point_Reference.md)

      + [C++ 组合数据类型]

      + [C++ 错误/异常](./CPP-common_exceptions.md)

      + [C++ 标准模板库(STL)]  

      + [C++ 标准库]  

        + [\<iostream\>](./CPPLIB-iostream.md)

      + [C++ 第三方程序库]  

      + [C++ 面向对象]  

      + [C++ 设计模式]

      + [C++ 主题讨论]

        + [输入输出，文件系统](./CPP-input_and_output.md)

      + [C++ IDE]

        + [Microsoft Visual Studio](./CPPIDE_VS-Settings-2512.md)

        + [Visual Studio Code](./CPPIDE_VSCode-Settings-2512.md)

        + [CodeLite](./CPPIDE_CodeLite-Settings-2512.md)

        + [QtCreator](./CPPIDE_QtCreator-Settings-2512.md)

      + [C++ 程序调试与排错]
      
        + [程序错误](./CPP-common_exceptions.md)

        + [程序调试](./CPP_tips-4debuginfo.md)  

        + [程序追踪]

          + [GDB]

          + [valgrind]

      + [C++ 知识点和技巧]

        + [C++ 知识点]

        + [C++ 技巧]

          + [添加调试信息](./CPP_tips-4debuginfo.md)

    + [C#]

    + [Java]

    + [Python]

    + [Shell Script]

    + [Batch Script & PowerShell]

  + [数据库]

    + [MySQL]

      + [数据类型](./MySQL-data_type.md)

    + [Oracle](./Oracle_CommonKnowledge.md)

      + [Oracle常见错误及处理](./Oracle_ErrorCode.md)

    + [Sqlite]

  + [数据结构]

    + [主题讨论]

  + [其他]

    + [git, github, gitee, gitlib](./github_gitee.md)

    + [云计算]

+ [知识速查]

  + [常用知识](./IT-CommonKnowledge.md)

  + [常用Code](./Codes.md)

  + [vi & vim](./vim-operatings.md)
